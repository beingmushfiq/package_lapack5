...
export default function BlogSection() {
  ...
  if (isLoading) {
    return <div className="h-48 bg-gray-100 animate-pulse m-8 rounded"></div>;
  }
              ...
              <div className="p-3 sm:p-5 flex-1 flex flex-col">
                <div className="flex items-center gap-2 sm:gap-4 mb-2 sm:mb-3 text-[8px] sm:text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="hidden xs:inline">{post.date || post.published_at || 'Recent'}</span>
                    <span className="xs:hidden">{(post.date || post.published_at || 'Recent').split(',')[0]}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <User className="w-2.5 h-2.5 sm:w-3 h-3" />
                    <span className="line-clamp-1">{(post.author || 'Admin').split(' ')[0]}</span>
                  </div>
                </div>
